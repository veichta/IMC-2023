...
from .refine_colmap import PixSfM as PixSfM_colmap
from .refine_colmap import add_common_args
from .util.hloc import (
    read_image_pairs,
    read_keypoints_hloc,
    read_matches_hloc,
    write_keypoints_hloc,
)
    ...
    def run_reconstruction(
        ...
        **hloc_args,
        ...
        if reference_model_path is None:
            ...
            #     model_path, image_dir, pairs_path, keypoints_path, matches_path, **hloc_args
            # )
            proc = subprocess.Popen(
                [
                    "python",
                    "hloc_reconstruction.py",
                    "--model_path",
                    str(model_path),
                    "--image_dir",
                    str(image_dir),
            ...
            try:
                logger.info(
                    "Running hloc reconstruction in subprocess "
                    + "(no console output until PixSfM finishes)"
                )
                output, error = proc.communicate()
                logger.info(output.decode())
                logger.error(error.decode())
            except Exception:
                logger.warning("Failed to run hloc reconstruction in subprocess.")
        else:
...
def add_hloc_args(parser):
    parser.add_argument(
        "--features_path", type=Path, required=True, help="hloc HDF5 file with the input keypoints."
    )
    parser.add_argument(
        "--pairs_path", type=Path, required=True, help="Text file with the list of image pairs."
    )
    parser.add_argument(
        "--matches_path", type=Path, required=True, help="hloc HDF5 file with the input matches."
    )
...
if __name__ == "__main__":
    ...
    parser_rec.add_argument("--sfm_dir", type=Path, required=True, help="Output SfM model.")
    ...
    parser_tri.add_argument(
        "--sfm_dir", type=Path, required=True, help="Path to the output SfM model."
    )
    parser_tri.add_argument(
        "--reference_sfm_model", type=Path, required=True, help="Path to the reference model."
    )
